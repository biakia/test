...
var Slideshow = {
	 ...
	 */
	fade: function(index) {
		Slideshow.slides.stop(true, true).fadeOut(3000);
		Slideshow.slides.eq(index).fadeIn(4000);
		...
		caption.stop(true, true).fadeOut('fast', function() {
			if (Slideshow.data[index]) {
				caption.html("")
					.append('<font size="22pt" color="white">'+ Slideshow.data[index].title +'</font>')
    ...
    play: function() {
        ...
        if (!Slideshow.playing) {
            ...
            Slideshow.timer = window.setInterval(Slideshow.rotate, 7000);
        }
    },
